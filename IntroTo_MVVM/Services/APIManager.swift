...
//  ShareCity
//
//  Created by App Knit on 27/05/19.
...
public class APIManager {
    ...
    //MARK:- I AM COOL
    func I_AM_COOL(vc: UIViewController,params: [String: Any],api: String,Loader: Bool,isMultipart:Bool,_ completion: @escaping (_ dictArr: Data?) -> Void){
        if !APIManager.isConnectedToNetwork()
        {
            APIManager().networkErrorMsg()
            return
        ...
        if Loader == true{
            DispatchQueue.main.async {
               vc.view.sainiShowLoader(loaderColor: AppColors.LoaderColor, backgroundColor: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 0.5))
            }
        }
